...
@interface WPSelfLocationController () <MAMapViewDelegate, AMapSearchDelegate,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,UISearchDisplayDelegate>
{
    //为了实现搜索
...
@implementation WPSelfLocationController
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (tableView != _tableView) {
        ...
        if (self.getLocationBlock) {
            self.getLocationBlock(_searchResultArray[indexPath.row][@"name"]);
        }
    } else {
        if (indexPath.row == 0) {
            //[self.delegate sendBackLocationWith:@"默认不显示"];
            if (self.getLocationBlock) {
                self.getLocationBlock(@"默认不显示");
            }
        } else if (indexPath.row == 1) {
            NSUserDefaults *user = [NSUserDefaults standardUserDefaults];
            ...
            if (self.getLocationBlock) {
                self.getLocationBlock(adress);
            }
        } else {
            //[self.delegate sendBackLocationWith:_names[indexPath.row - 2][@"name"]];
            if (self.getLocationBlock) {
                self.getLocationBlock(_names[indexPath.row - 2][@"name"]);
            }
        }
    }
