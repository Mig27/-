...
//    self.timeLabel.backgroundColor = [UIColor redColor];
    ...
    self.subTitleLabel.activeLinkTextAttributes = @{NSForegroundColorAttributeName:AttributedColor,NSBackgroundColorAttributeName:WPGlobalBgColor};
...
//    NSString *speak_comment_content = [WPMySecurities textFromEmojiString:dic[@"speak_comment_content"]];
    if (by_nick_name.length == 0) {
        NSString * string = dic[@"speak_comment_content"];
        commentStr = [NSString stringWithFormat:@"%@",[WPMySecurities textFromEmojiString:string.length?string:@""]];//dic[@"speak_comment_content"]
        commentStr = [WPMySecurities textFromBase64String:commentStr];
        if (!commentStr.length) {
            commentStr = [NSString stringWithFormat:@"%@",[WPMySecurities textFromEmojiString:string.length?string:@""]];
        }
        ...
        if (!string.length) {
            string = dic[@"speak_comment_content"];
            string = [WPMySecurities textFromEmojiString:string];
        }
...
{
    static NSString *cellId = @"NewCommentCellID";
    ...
    if (cell == nil) {
        ...
        cell = [[NewCommentCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
...
+ (CGFloat)cellHeightWith:(NSDictionary *)dic
{
    ...
    string = [WPMySecurities textFromBase64String:string];
    if (!string.length) {
        string = [WPMySecurities textFromEmojiString:dic[@"speak_comment_content"]];
    }
