...
@interface FreerideViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate,UIScrollViewDelegate,UITextViewDelegate,UITextFieldDelegate,sendBackLocation,WPActionSheet>
...
@property (nonatomic,strong) UILabel *StartingLabel;
@property (nonatomic,strong) UITextField *startingText;
@property (nonatomic,strong) UITextField *destinationField;
@property (nonatomic,strong) UILabel *timeLabel;
@property (nonatomic,strong) UITextField *numberText;
@property (nonatomic,strong) UITextField *styleText;
...
{
    NSLog(@"发布");
    if (self.startingText.text.length == 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"请填写出发地" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
    
    if (self.destinationField.text.length == 0) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"请填写目的地" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
    ...
    if (self.button5.isSelected) {
        if (self.styleText.text.length == 0) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"请填写车型" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
            [alert show];
            return;
        }
    }
    ...
    if (self.button4.isSelected) {
        ...
        params[@"models"] = self.styleText.text;
        params[@"plates"] = @"";
    }
...
//    NSString *url = [IPADDRESS stringByAppendingString:@"/ios/wonderful_demand.ashx"];
    ...
    NSDictionary *dic2 = @{@"title"  : @"出  发  地:",
                           @"content": self.startingText.text};
    
    [data addObject:dic2];
    
    NSDictionary *dic3 = @{@"title"  : @"目  的  地:",
                           @"content": self.destinationField.text};
    
    [data addObject:dic3];

    NSDictionary *dic4 = @{@"title"  : @"时        间:",
    ...
    if (self.button5.isSelected) {
        NSDictionary *dic6 = @{@"title"  : @"车       型:",
                               @"content": self.styleText.text};
        
        [data addObject:dic6];
        
        if (self.button1.isSelected) {
            NSDictionary *dic7 = @{@"title"  : @"要        求:",
                                   @"content": @"女"};
...
- (void)createUI
{
    ...
    scrollView.contentSize = CGSizeMake(SCREEN_WIDTH, 106 + WIDTH + 15 + 43*7 + 60 + 4 + 88 + 44);
    ...
    UIView *firstView = [[UIView alloc] initWithFrame:CGRectMake(0, 10, SCREEN_WIDTH, 43)];
    firstView.backgroundColor = [UIColor whiteColor];
    [scrollView addSubview:firstView];
    
    UILabel *firstLabel = [ZCControl createLabelWithFrame:CGRectMake(10, y, width, height) Font:15 Text:@"主        题:"];
    //    titleLabel.backgroundColor = [UIColor redColor];
    [firstView addSubview:firstLabel];

    UILabel *firstLabel2 = [ZCControl createLabelWithFrame:CGRectMake(firstLabel.right + RightF, y, SCREEN_WIDTH - 100, height) Font:15 Text:@"顺风车"];
    [firstView addSubview:firstLabel2];
    ...
    [firstView addSubview:themeControl];

    
    //主题
    UIView *themeView = [[UIView alloc] initWithFrame:CGRectMake(0, firstView.bottom + 1, SCREEN_WIDTH, 43)];
    themeView.backgroundColor = [UIColor whiteColor];
    [scrollView addSubview:themeView];
    
    UILabel *titleLabel = [ZCControl createLabelWithFrame:CGRectMake(10, y, width, height) Font:15 Text:@"类        型:"];
    //    titleLabel.backgroundColor = [UIColor redColor];
    [themeView addSubview:titleLabel];
    
    
    CGSize size2 = [@"搭顺风车" sizeWithAttributes:@{NSFontAttributeName:GetFont(15)}];
    UIButton *take = [UIButton buttonWithType:UIButtonTypeCustom];
//        take.backgroundColor = [UIColor redColor];
    take.titleLabel.font = [UIFont systemFontOfSize:15];
    take.frame = CGRectMake(titleLabel.right + RightF, 0, size2.width + 20, 43);
    take.titleLabel.textAlignment = NSTextAlignmentLeft;
    [take setTitleEdgeInsets:UIEdgeInsetsMake(y,  - 28, y, 0)];
    [take setImageEdgeInsets:UIEdgeInsetsMake(11.5, take.frame.size.width - 20, 11.5, -28)];
    [take setTitle:@"搭顺风车" forState:UIControlStateNormal];
    take.selected = YES;
    [take setTitleColor:RGBColor(170, 170, 170) forState:UIControlStateNormal];
    [take setTitleColor:[UIColor blackColor] forState:UIControlStateSelected];
    [take setImage:[UIImage imageNamed:@"menu_right_white"] forState:UIControlStateNormal];
    [take setImage:[UIImage imageNamed:@"menu_right_blue"] forState:UIControlStateSelected];
    take.tag = 4;
    [take addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
    self.button4 = take;
    [themeView addSubview:take];
    
    UIButton *set = [UIButton buttonWithType:UIButtonTypeCustom];
//        set.backgroundColor = [UIColor redColor];
    set.titleLabel.font = [UIFont systemFontOfSize:15];
    set.frame = CGRectMake(take.right + 24, 0, size2.width + 20, 43);
    set.titleLabel.textAlignment = NSTextAlignmentLeft;
    [set setTitleEdgeInsets:UIEdgeInsetsMake(y,  - 28, y, 0)];
    [set setImageEdgeInsets:UIEdgeInsetsMake(11.5, set.frame.size.width - 20, 11.5, -28)];
    [set setTitle:@"顺风车" forState:UIControlStateNormal];
    [set setTitleColor:RGBColor(170, 170, 170) forState:UIControlStateNormal];
    [set setTitleColor:[UIColor blackColor] forState:UIControlStateSelected];
    [set setImage:[UIImage imageNamed:@"menu_right_white"] forState:UIControlStateNormal];
    [set setImage:[UIImage imageNamed:@"menu_right_blue"] forState:UIControlStateSelected];
    set.tag = 5;
    [set addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
    self.button5 = set;
    [themeView addSubview:set];
    ...
    UILabel *companyTitle = [ZCControl createLabelWithFrame:CGRectMake(FRAME_X, y, width, height) Font:15 Text:@"出  发  地:"];
    [company addSubview:companyTitle];
    
    _startingText = [[UITextField alloc] initWithFrame:CGRectMake(companyTitle.right + RightF, 6.5, SCREEN_WIDTH - 44 - width, 30)];
    _startingText.placeholder = @"请输入出发地";
    [_startingText setValue:RGBColor(170, 170, 170) forKeyPath:@"_placeholderLabel.textColor"];
    [_startingText setValue:[UIFont boldSystemFontOfSize:15] forKeyPath:@"_placeholderLabel.font"];
    _startingText.font = [UIFont systemFontOfSize:15];
    [company addSubview:_startingText];

    
//    UILabel *companyLabel2 = [ZCControl createLabelWithFrame:CGRectMake(companyTitle.right + 24, y, SCREEN_WIDTH - 54 - width, height) Font:15 Text:@"请选择出发地"];
//    companyLabel2.textColor = RGBColor(170, 170, 170);
//    self.StartingLabel = companyLabel2;
...
//    
//    UIControl *StartingControl = [[UIControl alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, NORMALHEIGHT)];
//    StartingControl.tag = 1;
//    [StartingControl addTarget:self action:@selector(clickToSelectWith:) forControlEvents:UIControlEventTouchUpInside];
//    [company addSubview:StartingControl];
    ...
    UILabel *industryLabel1 = [ZCControl createLabelWithFrame:CGRectMake(FRAME_X, y, width, height) Font:15 Text:@"目  的  地:"];
    [industry addSubview:industryLabel1];
    
    UITextField *destination = [[UITextField alloc] initWithFrame:CGRectMake(industryLabel1.right + RightF, 6.5, SCREEN_WIDTH - 44 - width, 30)];
    self.destinationField = destination;
    destination.placeholder = @"请输入目的地";
    [destination setValue:RGBColor(170, 170, 170) forKeyPath:@"_placeholderLabel.textColor"];
    [destination setValue:[UIFont boldSystemFontOfSize:15] forKeyPath:@"_placeholderLabel.font"];
    destination.font = [UIFont systemFontOfSize:15];
    [industry addSubview:destination];

    
//    UILabel *industryLabel2 = [ZCControl createLabelWithFrame:CGRectMake(industryLabel1.right + 24, y, SCREEN_WIDTH - 54 - width, height) Font:15 Text:@"请选择目的地"];
...
//    [industry addSubview:industryImg];
    ...
    //时间
    UIView *position = [[UIView alloc] initWithFrame:CGRectMake(0, industry.bottom + 10, SCREEN_WIDTH, 43)];
    position.backgroundColor = [UIColor whiteColor];
    self.timeView = position;
    [scrollView addSubview:position];
    
    UILabel *positionLable1 = [ZCControl createLabelWithFrame:CGRectMake(FRAME_X, y, width, height) Font:15 Text:@"时        间:"];
    [position addSubview:positionLable1];
    
    UILabel *positionLabel2 = [ZCControl createLabelWithFrame:CGRectMake(industryLabel1.right + RightF, y, SCREEN_WIDTH - 54 - width, height) Font:15 Text:@"请选择时间"];
    positionLabel2.textColor = RGBColor(170, 170, 170);
    self.timeLabel = positionLabel2;
    ...
    [position addSubview:timeControl];
    ...
    UITextField *number = [[UITextField alloc] initWithFrame:CGRectMake(salaryLabel1.right + RightF, 6.5, SCREEN_WIDTH - 44 - width, 30)];
    self.numberText = number;
    number.placeholder = @"请填写人数";
    number.tag = 5;
    number.delegate = self;
    number.keyboardType = UIKeyboardTypeNumberPad;
    [number setValue:RGBColor(170, 170, 170) forKeyPath:@"_placeholderLabel.textColor"];
    [number setValue:[UIFont boldSystemFontOfSize:15] forKeyPath:@"_placeholderLabel.font"];
    number.font = [UIFont systemFontOfSize:15];
    [salary addSubview:number];
...
//    [salary addSubview:salaryImg];
    ...
    UILabel *experienceLabel1 = [ZCControl createLabelWithFrame:CGRectMake(FRAME_X, y, width, height) Font:15 Text:@"车       型:"];
    [experience addSubview:experienceLabel1];
    
    UITextField *style = [[UITextField alloc] initWithFrame:CGRectMake(experienceLabel1.right + RightF, 6.5, SCREEN_WIDTH - 44 - width, 30)];
    self.styleText = style;
    style.tag = 10;
    style.delegate = self;
    style.placeholder = @"请填写车型";
    [style setValue:RGBColor(170, 170, 170) forKeyPath:@"_placeholderLabel.textColor"];
    [style setValue:[UIFont boldSystemFontOfSize:15] forKeyPath:@"_placeholderLabel.font"];
    style.font = [UIFont systemFontOfSize:15];
    [experience addSubview:style];
    self.styleView = experience;

    
//    UILabel *experienceLabel2 = [ZCControl createLabelWithFrame:CGRectMake(industryLabel1.right + 24, y, SCREEN_WIDTH - 54 - width, height) Font:15 Text:@"请选择车型"];
...
//    [experience addSubview:experienceImg];
    ...
    //要求
    UIView *require = [[UIView alloc] initWithFrame:CGRectMake(0, experience.bottom + 1, SCREEN_WIDTH, 43)];
...
//    
//    UILabel *locationLabel2 = [ZCControl createLabelWithFrame:CGRectMake(industryLabel1.right + 24, y, SCREEN_WIDTH - 54 - width, height) Font:15 Text:@"请选择工作地点"];
...
//    [location addSubview:locationImg];
    ...
    UIView *cost = [[UIView alloc] initWithFrame:CGRectMake(0, salary.bottom + 1, SCREEN_WIDTH, 43)];
    cost.backgroundColor = [UIColor whiteColor];
    [scrollView addSubview:cost];
    
    UILabel *costLabel1 = [ZCControl createLabelWithFrame:CGRectMake(FRAME_X, y, width, height) Font:15 Text:@"费        用:"];
    [cost addSubview:costLabel1];
    
    UIButton *pay = [UIButton buttonWithType:UIButtonTypeCustom];
    //        unlimited.backgroundColor = [UIColor redColor];
    pay.titleLabel.font = [UIFont systemFontOfSize:15];
    pay.frame = CGRectMake(costLabel1.right + RightF, 0, 2*size1.width + 26, 43);
    pay.titleLabel.textAlignment = NSTextAlignmentLeft;
    [pay setTitleEdgeInsets:UIEdgeInsetsMake(y,  - 26, y, girl.bounds.size.width - 24)];
    [pay setImageEdgeInsets:UIEdgeInsetsMake(11.5, girl.frame.size.width - 20, 11.5, -40)];
    [pay setTitle:@"付费" forState:UIControlStateNormal];
    [pay setTitleColor:RGBColor(170, 170, 170) forState:UIControlStateNormal];
    [pay setTitleColor:[UIColor blackColor] forState:UIControlStateSelected];
    [pay setImage:[UIImage imageNamed:@"menu_right_white"] forState:UIControlStateNormal];
    [pay setImage:[UIImage imageNamed:@"menu_right_blue"] forState:UIControlStateSelected];
    pay.tag = 6;
    pay.selected = YES;
    [pay addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
    self.button6 = pay;
    [cost addSubview:pay];
    
    UIButton *free = [UIButton buttonWithType:UIButtonTypeCustom];
    //        unlimited.backgroundColor = [UIColor redColor];
    free.titleLabel.font = [UIFont systemFontOfSize:15];
    free.frame = CGRectMake(pay.right + 24, 0, 2*size1.width + 26, 43);
    free.titleLabel.textAlignment = NSTextAlignmentLeft;
    [free setTitleEdgeInsets:UIEdgeInsetsMake(y,  - 26, y, girl.bounds.size.width - 24)];
    [free setImageEdgeInsets:UIEdgeInsetsMake(11.5, girl.frame.size.width - 20, 11.5, -40)];
    [free setTitle:@"免费" forState:UIControlStateNormal];
    [free setTitleColor:RGBColor(170, 170, 170) forState:UIControlStateNormal];
    [free setTitleColor:[UIColor blackColor] forState:UIControlStateSelected];
    [free setImage:[UIImage imageNamed:@"menu_right_white"] forState:UIControlStateNormal];
    [free setImage:[UIImage imageNamed:@"menu_right_blue"] forState:UIControlStateSelected];
    free.tag = 7;
    [free addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
    self.button7 = free;
    [cost addSubview:free];
    self.costView = cost;

    
    //就职时间
//    UIView *time = [[UIView alloc] initWithFrame:CGRectMake(0, require.bottom + 1, SCREEN_WIDTH, 43)];
...
//    
//    UILabel *timeLabel2 = [ZCControl createLabelWithFrame:CGRectMake(industryLabel1.right + 24, y, SCREEN_WIDTH - 54 - width, height) Font:15 Text:@"请选择就职时间"];
...
//    [time addSubview:timeImg];
    ...
    UIPlaceHolderTextView *text = [[UIPlaceHolderTextView alloc] initWithFrame:CGRectMake(5, 0, SCREEN_WIDTH - 10, 103 - 10)];
    text.placeholder = @"备        注:  说些什么...";
    text.font = [UIFont systemFontOfSize:15];
    text.delegate = self;
    self.text = text;
    [remark addSubview:text];
...
//    UIBarButtonItem * button1 = [[UIBarButtonItem alloc]initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:self action:@selector(resignKeyboard)];
    ...
    [_startingText setInputAccessoryView:topView];
    [_destinationField setInputAccessoryView:topView];
    [_numberText setInputAccessoryView:topView];
    [_styleText setInputAccessoryView:topView];
}
...
- (void)textFieldDidBeginEditing:(UITextField *)textField
{
    if (textField.tag == 5) {
        ...
        CGRect textFrame =  self.styleView.frame;
        float textY = textFrame.origin.y+textFrame.size.height;
        float bottomY = self.view.frame.size.height- (textY - _scroll.contentOffset.y);
        if (bottomY>=240 + 30) {
            return;
...

{
    ...
    CGRect textFrame =  self.remarkView.frame;
    float textY = textFrame.origin.y+93;
    float bottomY = self.view.frame.size.height- (textY - _scroll.contentOffset.y);
    NSLog(@"%f",bottomY);
...
{
    if (self.button4.isSelected) {
        self.costView.frame = CGRectMake(0, self.numberView.bottom + 1, SCREEN_WIDTH, 43);
        self.scroll.contentSize = CGSizeMake(SCREEN_WIDTH, 106 + WIDTH + 15 + 43*7 + 60 + 4 + 88 + 44);
    } else {
        self.costView.frame = CGRectMake(0, self.numberView.bottom + 1 + 88, SCREEN_WIDTH, 43);
        self.scroll.contentSize = CGSizeMake(SCREEN_WIDTH, 106 + WIDTH + 15 + 43*7 + 60 + 4 + 88 + 88 + 44);
    }
    
    self.remarkView.frame = CGRectMake(0, self.costView.bottom + 1, SCREEN_WIDTH, 106 + WIDTH + 15);
    self.locationView.frame = CGRectMake(0, self.remarkView.bottom + 10, SCREEN_WIDTH, 43);
    self.anonymousView.frame = CGRectMake(0, self.locationView.bottom + 1, SCREEN_WIDTH, 43);
    self.shareView.frame = CGRectMake(0, self.anonymousView.bottom + 1, SCREEN_WIDTH, 43);
}
