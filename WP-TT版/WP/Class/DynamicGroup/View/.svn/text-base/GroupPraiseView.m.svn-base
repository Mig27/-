...
@interface GroupPraiseView ()

@end

@implementation GroupPraiseView
...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i<self.praiseArr.count; i++) {
        PraiseListModel *model = self.praiseArr[i];
//        NSDictionary *dic = self.praiseArr[i];
        NSString *nick_name = model.created_nick_name;
    ...
    imageV.image = [UIImage imageNamed:@"zhichangshuoshuo_zanren"];
...
//    10
    ...
    [linkLabel setDidClickLinkBlock:^(MLLink *link, NSString *linkText, MLLinkLabel *label) {
        NSLog(@"点击赞的人");
        PraiseListModel *info = self.praiseArr[link.linkValue.integerValue];
        [[NSNotificationCenter defaultCenter] postNotificationName:@"alumPersonal" object:nil userInfo:@{@"user_id" : info.created_user_id,@"nick_name" : info.created_nick_name}];
