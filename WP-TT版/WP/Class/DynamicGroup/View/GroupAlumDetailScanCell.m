...
@implementation GroupAlumDetailScanCell
...
- (void)createUI
{
    ...
    self.nickLabel.textColor = [UIColor blackColor];//AttributedColor
...
{
    self.timeLabel.text = dic.read_time.length?dic.read_time:dic.add_time;
...
{
    static NSString *cellId = @"GroupAlumDetailScanCellID";
    GroupAlumDetailScanCell *cell = [tableView dequeueReusableCellWithIdentifier:cellId];
    if (cell == nil) {
        //        cell = [[NSBundle mainBundle] loadNibNamed:@"NearActivityCell" owner:self options:nil][0];
        cell = [[GroupAlumDetailScanCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellId];
    }
