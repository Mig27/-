...
@implementation WPUpdateFriendStatusHttp
+ (void)WPUpdateFriendStatusHttpWithParam:(WPUpdateFriendStatusparam *)param success:(void (^)(WPUpdateFriendStatusResult *result))success failure:(void (^)(NSError *error))failure{
    ...
    if (param.VerState) {
        parame[@"VerState"]= param.VerState;
    }
    if (param.friend_id) {
    ...
    [mgr POST:url parameters:parame success:^(AFHTTPRequestOperation *operation, id responseObject) {
        WPUpdateFriendStatusResult *result = [WPUpdateFriendStatusResult mj_objectWithKeyValues:responseObject];
