//
//  WPPhoneBookRecruitSetController.m
//  WP
//
...
#import "WPDontLetHeSeeMyRecruitHttp.h"
...
@interface WPPhoneBookRecruitSetController ()<UITableViewDelegate,UITableViewDataSource,UIAlertViewDelegate>

@property (nonatomic,strong) UITableView *tableView;

@end

@implementation WPPhoneBookRecruitSetController
...
-(void)dontLetHeSee:(id)sender{
    ...
    WPDontLetHeSeeMyRecruitParam *param = [[WPDontLetHeSeeMyRecruitParam alloc] init];
    param.action = @"AddDontSeeJobFromHe";
    ...
    [WPDontLetHeSeeMyRecruitHttp WPDontLetHeSeeMyRecruitHttpWithParam:param success:^(WPDontLetHeSeeMyRecruitResult *result) {
...
-(void)dontSeeHisRecruit:(id)sender{
    WPShareModel *model = [WPShareModel sharedModel];
    NSMutableDictionary *userInfo = model.dic;
    WPDontSeeHisRecruitParam *param = [[WPDontSeeHisRecruitParam alloc] init];
    param.action = @"AddDontSeeJobToHe";
    param.username = model.username;
    param.password = model.password;
    param.user_id = userInfo[@"userid"];
    ...
    [WPDontSeeHisRecruitHttp WPDontSeeHisRecruitHttpWithParam:param success:^(WPDontSeeHisRecruitResult *result) {
        if (result.status.intValue == 0) {
        }else{
            [MBProgressHUD showError:result.info];
        }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        ...
        WPPhonebookSwitchCell *cell= [WPPhonebookSwitchCell cellWithTableView:tableView];
        cell.titleLbl.text = @"不看他(她)的招聘";
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        [cell.switchView addTarget:self action:@selector(dontSeeHisRecruit:) forControlEvents:UIControlEventValueChanged];
        if ([self.result.is_job isEqualToString:@"1"]) {
            [cell.switchView setOn:YES];
        }else{
            [cell.switchView setOn:NO];
        }
