...
@interface WPCategoryDetailController () <UITextFieldDelegate, UISearchBarDelegate, UISearchDisplayDelegate, UITabBarDelegate, UITableViewDataSource,UITableViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,WPActionSheet,UISelectDelegate,WPInterviewControllerDelegate,WPRecuilistControllerDelegate>
...
@implementation WPCategoryDetailController
...
- (void)viewDidLoad {
    ...
    _model.areaID = @"";
    _model.industryID = (self.industryId?self.industryId:@"");
    _model.Position = @"";
    _model.Sex = @"1";
    _model.Wage = @"1";
    _model.Experience = @"1";
...
- (UITableView*)tableView2
{
    if (_tableView2 == nil) {
        ...
        //        self.tableView2.separatorColor = WPColor(226, 226, 226);
...
- (UIScrollView*)scrollViewFirst
{
    if (_scrollViewFirst == nil) {
        ...
        NSArray* arrayButtonName = @[ @"职称", @"工资", @"经验",@"学历",@"福利",@"年龄", @"性别"];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_isRecruit) {
        ...
        interView.urlStr = [NSString stringWithFormat:@"%@/webMobile/November/EnterpriseRecruit.aspx?recruit_id=%@",IPADDRESS,model.sid];
    }else{
        NSLog(@"求职");
        ...
        WPShareModel *shareModel = [WPShareModel sharedModel];
        interView.isSelf = [model.userId isEqualToString:shareModel.dic[@"userid"]];
        interView.urlStr = [NSString stringWithFormat:@"%@/webMobile/November/resume_info.aspx?resume_id=%@",IPADDRESS,model.jobId];
    }
}
...
- (void)detailActions:(NSIndexPath *)indexPath
{
    ...
    if (_isRecruit) {
        ...
        WPShareModel *shareModel = [WPShareModel sharedModel];
        interView.isSelf = [model.userId isEqualToString:shareModel.dic[@"userid"]];
        interView.urlStr = [NSString stringWithFormat:@"%@/webMobile/November/recruitCompany.aspx?ep_id=%@",IPADDRESS,model.epId];
    }
    //    else{
    //        NSLog(@"求职");
    //        WPInterviewListModel *model = [[WPInterviewListModel alloc]init];
