...
@interface WPSelfViewController () <UITextFieldDelegate, UISearchBarDelegate, UISearchDisplayDelegate, UITabBarDelegate, UITableViewDataSource,UITableViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,WPActionSheet,UISelectDelegate,WPInterviewControllerDelegate,WPRecuilistControllerDelegate>
...
@implementation WPSelfViewController
...
- (void)viewDidLoad {
    ...
    _model.action = @"RecommendYourself";
    _model.areaID = @"";
    _model.industryID = @"";
    _model.Position = @"";
    _model.Sex = @"1";
...
-(void)requestWithPageIndex:(int)pageIndex Success:(DealsSuccessBlock)success Error:(DealsErrorBlock)errors
{
    ...
    NSString *str = [IPADDRESS stringByAppendingString:@"/ios/resume.ashx"];
    ...
    [WPHttpTool postWithURL:str params:params success:^(id json) {
        ...
        if (_isRecruit) {/**< 招聘 */
            ...
            WPRecommendYourselfModel *recruitModel = [WPRecommendYourselfModel mj_objectWithKeyValues:json];
            NSArray *arr = [[NSArray alloc]initWithArray:recruitModel.list];
            success(arr,(int)arr.count);
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_isRecruit) {
        ...
        NSLog(@"求职");
        WPRecommendYourselfListModel *model = [[WPRecommendYourselfListModel alloc]init];
        switch (_bottomType) {
            case 0:
        ...
        WPShareModel *shareModel = [WPShareModel sharedModel];
        interView.isSelf = [model.userId isEqualToString:shareModel.dic[@"userid"]];
        interView.urlStr = [NSString stringWithFormat:@"%@/webMobile/November/resume_info.aspx?resume_id=%@&user_id=%@",IPADDRESS,model.sid,kShareModel.userId];
    }
}
