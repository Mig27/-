...
@interface WPIndustryViewController () <UITextFieldDelegate, UISearchBarDelegate, UISearchDisplayDelegate, UITabBarDelegate, UITableViewDataSource,UITableViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,WPActionSheet,UISelectDelegate,WPInterviewControllerDelegate,WPRecuilistControllerDelegate,UIGestureRecognizerDelegate>
...
@implementation WPIndustryViewController
...
- (UIScrollView*)scrollViewFirst
{
    if (_scrollViewFirst == nil) {
        ...
        NSArray* arrayButtonName = @[(self.industryName?self.industryName:@"行业"), @"职称", @"工资", @"经验",@"学历",@"福利",@"年龄", @"性别"];
        ...
        if(_selectedButton == sender){
            ...
            switch (sender.tag-10) {
                ...
                case 2:/**< 工资 */
                    ...
                    break;
                case 3:/**< 经验 */
                    [self.city setLocalData:[SPLocalApplyArray workTimeArray]];
                    break;
                case 4:/**< 学历 */
...
- (void)detailActions:(NSIndexPath *)indexPath
{
    ...
    WPShareModel *shareModel = [WPShareModel sharedModel];
    interView.isSelf = [model.userId isEqualToString:shareModel.dic[@"userid"]];
    interView.urlStr = [NSString stringWithFormat:@"%@/webMobile/November/recruitCompany.aspx?ep_id=%@&user_id=%@",IPADDRESS,model.epId,kShareModel.dic[@"userid"]];
}
