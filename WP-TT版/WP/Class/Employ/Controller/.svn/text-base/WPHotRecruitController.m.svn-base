...
@interface WPHotRecruitController ()<UITextFieldDelegate, UISearchBarDelegate, UISearchDisplayDelegate, UITabBarDelegate, UITableViewDataSource,UITableViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource,WPActionSheet,UISelectDelegate,WPInterviewControllerDelegate,WPRecuilistControllerDelegate>
...
@implementation WPHotRecruitController
...
- (void)viewDidLoad {
    ...
    _model.action = @"SeekMaxima";
    _model.areaID = @"";
    _model.industryID = @"";
    _model.Position = self.hotPosition?self.hotPosition:@"";
    _model.Sex = @"1";
    _model.Wage = @"1";
    _model.Experience = @"1";
    _model.education = @"";
    _model.Welfare = self.hotArea?self.hotArea:@"1";
    _model.age = @"";
...
- (void)setNavbarItem/**< 初始化导航栏 */
{
    ...
    button1.frame = CGRectMake(0, 0, 35, 22);
    ...
    [button1 addTarget:self action:@selector(rightBarButtonItemClick:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [button setImage:[UIImage imageNamed:@"qiehuanliebiao"] forState:UIControlStateNormal];
    [button setImage:[UIImage imageNamed:@"tupianmushi"] forState:UIControlStateSelected];
    [button addTarget:self action:@selector(switchPhotoStyle:) forControlEvents:UIControlEventTouchUpInside];
    UIBarButtonItem *rightBarItem2 = [[UIBarButtonItem alloc]initWithCustomView:button];
...
{
    //    NSString *actionStr;
    if ([_model.action isEqualToString:@"SeekMaxima"]) {
        _isRecruit = YES;
    }else{
        _isRecruit = NO;
    }
    ...
    [WPHttpTool postWithURL:str params:params success:^(id json) {
        ...
        if (_isRecruit) {/**< 招聘 */
            WPSearchMaximaModel *recruitModel = [WPSearchMaximaModel mj_objectWithKeyValues:json];
            NSArray *arr = [[NSArray alloc]initWithArray:recruitModel.list];
            success(arr,(int)arr.count);
        }else{/**< 求职 */
            //            WPInterviewModel *recruitModel = [WPInterviewModel objectWithKeyValues:json];
            //            NSArray *arr = [[NSArray alloc]initWithArray:recruitModel.list];
            //            success(arr,(int)arr.count);
        }
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if (tableView == self.tableView4) {
        ...
        if (_isRecruit) {/**< 招聘 */
            WPSearchMaximaCell *cell = [WPSearchMaximaCell tableView:tableView cellForRowAtIndexPath:indexPath];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    interView.isRecuilist = _isRecruit;
    [self.navigationController pushViewController:interView animated:YES];
    if (_isRecruit) {
        NSLog(@"招聘");
        WPSearchMaximaListModel *model = [[WPSearchMaximaListModel alloc]init];;
        switch (_bottomType) {
            case 0:
        ...
        interView.urlStr = [NSString stringWithFormat:@"%@/webMobile/November/EnterpriseRecruit.aspx?recruit_id=%@&user_id=%@",IPADDRESS,model.sid,kShareModel.dic[@"userid"]];
    }else{
        NSLog(@"求职");
        ...
        WPShareModel *shareModel = [WPShareModel sharedModel];
        interView.isSelf = [model.userId isEqualToString:shareModel.dic[@"userid"]];
        interView.urlStr = [NSString stringWithFormat:@"%@/webMobile/November/resume_info.aspx?resume_id=%@@&user_id=%@",IPADDRESS,model.jobId,kShareModel.dic[@"userid"]];
    }
}
...
- (void)detailActions:(NSIndexPath *)indexPath
{
    ...
    if (_isRecruit) {
        ...
        WPShareModel *shareModel = [WPShareModel sharedModel];
        interView.isSelf = [model.userId isEqualToString:shareModel.dic[@"userid"]];
        interView.urlStr = [NSString stringWithFormat:@"%@/webMobile/recruitCompany.aspx?company_id=%@",IPADDRESS,model.sid];
    }
    //    else{
    //        NSLog(@"求职");
    //        WPInterviewListModel *model = [[WPInterviewListModel alloc]init];
