...
//    request.requireExtension    = YES;
    ...
    request.requireExtension    = YES;
    request.page = self.page;
    request.types = @"风景名胜|商务住宅|政府机构及社会团体|道路附属设施|交通设施服务";
...
- (void)onPOISearchDone:(AMapPOISearchBaseRequest *)request response:(AMapPOISearchResponse *)response
{
    ...
    {
        NSString * localCity = [[NSUserDefaults standardUserDefaults] objectForKey:@"localCity"];
        NSDictionary * dic = @{@"name":localCity,@"address":@""};
        [_names insertObject:dic atIndex:0];
        self.isFromShuoShuo = NO;
    }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    return (self.isFromCreatGroup?_names.count:_names.count + 1);
}
...
//            cell.detailTextLabel.text = nil;
            if (self.isFromCreatGroup)
            {
                ...
                if (_names.count)
                {
                    cell.nameLabel.text = _names[indexPath.row][@"name"];
                    cell.locationLabel.text = _names[indexPath.row][@"address"];
                }
                return cell;
            }
            else//不显示位置
            {
                LocationHeaderCell *cell = [LocationHeaderCell cellWithTableView:tableView];
                return cell;
            }
//            LocationHeaderCell *cell = [LocationHeaderCell cellWithTableView:tableView];
//            return cell;
...
//        }
            ...
            else
            {
                ...
                cell.nameLabel.text = _names[indexPath.row - 1][@"name"];
                cell.locationLabel.text = _names[indexPath.row - 1][@"address"];
                if ([cell.nameLabel.text isEqualToString:self.choiseStr])
                {
                    cell.choiseImage.hidden = NO;
                }
                else
                {
                    cell.choiseImage.hidden = YES;
                }
                ...
                if ([_names[indexPath.row - 1][@"name"] isEqualToString:string]) {
                    CGSize normalSize1 = [@"好友圈动" sizeWithAttributes:@{NSFontAttributeName:kFONT(15)}];
                    CGRect rect = cell.nameLabel.frame;
                    rect.origin.y = (kHEIGHT(58)-normalSize1.height)/2;
                    cell.nameLabel.frame = rect;
                    
                    cell.nameLabel.textColor = RGB(0, 172, 255);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    NSString * cityString = [[NSUserDefaults standardUserDefaults] objectForKey:@"localCity"];
    ...
    if (tableView != _tableView) {
        ...
        if (indexPath.row == 0) {
            if (self.isFromCreatGroup)
            {
              ...
              [self.delegate sendBackLocationWith:[NSString stringWithFormat:@"%@",_names[indexPath.row][@"name"]]];
            }
            else
            {
              [self.delegate sendBackLocationWith:@"不显示位置"];
            }
//            [self.delegate sendBackLocationWith:@"不显示位置"];
        }
//        else if (indexPath.row == 1) {
...
//        }
            ...
            if (self.isFromCreatGroup)
            {
                if ([cityString isEqualToString:_names[indexPath.row][@"name"]]) {
                   [self.delegate sendBackLocationWith:[NSString stringWithFormat:@"%@",_names[indexPath.row][@"name"]]];
                }
                else
                {
                   [self.delegate sendBackLocationWith:[NSString stringWithFormat:@"%@·%@",cityString,_names[indexPath.row][@"name"]]];
                }
...
//              [self.delegate sendBackLocationWith:[NSString stringWithFormat:@"%@%@",cityString,_names[indexPath.row][@"name"]]];
            ...
            {
                if ([cityString isEqualToString:_names[indexPath.row-1][@"name"]]) {
                    [self.delegate sendBackLocationWith:[NSString stringWithFormat:@"%@",_names[indexPath.row - 1][@"name"]]];
                }
                else
                {
                    [self.delegate sendBackLocationWith:[NSString stringWithFormat:@"%@·%@",cityString,_names[indexPath.row - 1][@"name"]]];
                }
                
//              [self.delegate sendBackLocationWith:[NSString stringWithFormat:@"%@%@",cityString,_names[indexPath.row - 1][@"name"]]];
            }
//            [self.delegate sendBackLocationWith:_names[indexPath.row - 1][@"name"]];
        }
    }
