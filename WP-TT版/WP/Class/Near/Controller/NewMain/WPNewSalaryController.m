...
@interface WPNewSalaryController ()<UISearchBarDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UISelectDelegate,WPInterviewControllerDelegate,WPRecuilistControllerDelegate,UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) WPNewSearchResumeListModel *selectedModel;
...
@implementation WPNewSalaryController
...
#pragma mark - 初始化数组
- (WPNewSearchResumeListModel *)selectedModel{
    if (!_selectedModel) {
        _selectedModel = [[WPNewSearchResumeListModel alloc]init];
    }
...
- (NearCategoryModel *)model{
    if (!_model) {
        ...
        _model.action = self.type == WPMainPositionTypeInterView?@"RecommendYourself":@"SeekMaxima";
        _model.areaID = @"";
        _model.industryID = @"";
        _model.Position = @"";
        _model.Sex = @"1";
...
-(UICollectionView *)collectionView
{
    if (!_collectionView) {
        ...
        [_collectionView registerClass:[WPNewFullSearchCell class] forCellWithReuseIdentifier:@"WPNewFullSearchCell"];
...
- (void)editBottomClick:(UIButton *)sender{
    ...
    NSString *str = @"";
    for (WPNewSearchResumeListModel *model in self.selectedArray) {
    ...
    if (sender.tag == 42) {
        ...
        if (self.selectedArray.count != 0) {
            NSString *resumeId = @"";
            for (WPNewSearchResumeListModel *listModel in self.selectedArray) {
...
-(void)requestWithPageIndex:(int)pageIndex Success:(DealsSuccessBlock)success Error:(DealsErrorBlock)errors
{
    ...
    [WPHttpTool postWithURL:str params:params success:^(id json) {
        ...
        WPNewSearchResumeModel *recruitModel = [WPNewSearchResumeModel mj_objectWithKeyValues:json];
        NSArray *arr = [[NSArray alloc]initWithArray:recruitModel.list];
        for (WPNewSearchResumeListModel *listModel in arr) {
...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_collectionViewItemIsFullScreen) {
        static NSString *cellId = @"WPNewFullSearchCell";
        WPNewFullSearchCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellId forIndexPath:indexPath];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_collectionViewItemIsFullScreen) {
        return CGSizeMake(SCREEN_WIDTH, kHEIGHT(71));
    }else{
        int width = (SCREEN_WIDTH == 320?320:(SCREEN_WIDTH == 375)?376:416);
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    WPNewSearchResumeListModel *model = self.resumeArr[indexPath.row];
