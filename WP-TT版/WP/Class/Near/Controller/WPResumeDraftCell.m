...
@implementation WPResumeDraftCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        _chooseBtn.frame = CGRectMake(0, 0, kHEIGHT(43), kHEIGHT(58));
...
- (void)btnAction:(UIButton *)sender
{
    ...
    NSString * string = [NSString stringWithFormat:@"%@",[numDefault objectForKey:@"numberOfChoise"]];//取出选中的个数
    ...
    [numDefault setObject:[NSString stringWithFormat:@"%d",num] forKey:@"numberOfChoise"];
...
- (void)setModel:(WPResumDraftModel *)model isEdit:(BOOL)edit
{
    ...
    if (!edit && model.itemIsSelected) {
        ...
        _chooseBtn.frame = CGRectMake( kScreen_Width - kHEIGHT(40),0, kHEIGHT(43), kHEIGHT(58));
       // [UIView animateWithDuration:0.3 animations:^{
         //   _iconImage.left = kHEIGHT(10)+kHEIGHT(10)+18;
           // _nameLbl.left = _iconImage.right + kHEIGHT(10);
            //_detailLbl.left = _iconImage.right+kHEIGHT(10);
    ...
    self.nameLbl.text = [@"求职：" stringByAppendingString:_model.Hope_Position];
    self.detailLbl.text = [NSString stringWithFormat:@"%@ %@ %@ %@ %@", _model.nickName, _model.sex, _model.birthday, _model.education, _model.worktime];
