...
-(void)creatUI
{
    ...
    UILabel *nameLabel =[[THLabel alloc]initWithFrame:CGRectMake(26 + kHEIGHT(54), kHEIGHT(9) + 4,(kScreen_Width - kHEIGHT(54) - 62), 20)];
...
#pragma mark  被移除 / 群主转移 / 申请加入群被拒绝 时显示的是群头像 其他是对应人头像
    ...
    {       //群成员消息 之 申请被拒绝
        if (state.intValue == 2)
        {
            ...
            NSString * denialString = self.infoDic[@"denial"];
            if (denialString.length > 0) {
                bottomLabel.text = [NSString stringWithFormat:@"群主拒绝了你的加群申请: %@", self.infoDic[@"denial"]];
            }else{
                bottomLabel.text = @"群主拒绝了你的加群申请";
            }
            
            separeLabel.hidden = YES;
            companyLabel.hidden = YES;
            positionLabel.text = [NSString stringWithFormat:@"群号码: %@",self.infoDic[@"group_no"]];
        }
    }
    ...
    {       //群成员被动消息：别人邀请我入群(满40人需验证)、被移出群组、群被解散、别人转移群主权限给我
        ...
        //头像
        if (state.intValue != 4 && state.intValue != 7) {
...
//
    ...
    //    }];
//    if (nameLabel.width > (kScreen_Width - kHEIGHT(54) - 62)) {
//        nameLabel.width  = (kScreen_Width - kHEIGHT(54) - 62);
//    }
}
...
-(void)clickBnt:(UIButton*)sender
{
    ...
    {
        //判断是否已经存在群组中了
        __block typeof(state) block_state = state;
        [self isExitOfGroup:self.infoDic[@"g_id"] success:^(id json) {
            if (!json) {
                [MBProgressHUD createHUD:@"获取信息失败" View:self.view];
                return ;
            }
            ...
            information.gtype = [block_state isEqualToString:@"0"]?@"2":([self.infoDic[@"status"] isEqualToString:@"7"]?@"1":@"3");
