        ...
        if ([json[@"status"] integerValue] == 0) {
            if (!self.isEditing) {//非编辑 --> 创建群公告
                ...
                [[self.navigationController viewControllers] enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull viewcontroller, NSUInteger idx, BOOL * _Nonnull stop) {
                    if ([NSStringFromClass(viewcontroller.class) isEqualToString:NSStringFromClass([WPGroupNoticeViewController class])]) {
                        NSLog(@"当前下标是 index = %@",_currentIndex);
                      [[NSNotificationCenter defaultCenter] postNotificationName:@"noticeUpdate" object:nil userInfo:@{@"index":_currentIndex}];
            ...
            //无论是创建群公告还是修改群公告都需要刷新,群信息详情页的群公告信息
