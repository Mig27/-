...
//        NSLog(@"%@",json);
        ...
        if (model.list.count) {
            _model = model.list[0];
            [self.tableView reloadData];
        }else if (self.noticeId.length){
            [MBProgressHUD createHUD:@"该内容已被删除" View:self.view];
    ...
    [WPHttpTool postWithURL:url params:params success:^(id json) {
        ...
        if ([json[@"status"] integerValue] == 0) {
            ...
            [[self.navigationController viewControllers] enumerateObjectsUsingBlock:^(__kindof UIViewController * _Nonnull viewcontroller, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([NSStringFromClass(viewcontroller.class) isEqualToString:NSStringFromClass([WPGroupInformationViewController class])]) {
