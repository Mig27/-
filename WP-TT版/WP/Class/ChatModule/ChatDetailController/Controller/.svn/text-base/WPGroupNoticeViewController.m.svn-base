...
@property (nonatomic ,strong) NSIndexPath * tempIndexPath;  //临时indexPath,防止通知丢值
...
{
    if ([notification.userInfo containsObjectForKey:@"index"]) {
        NSIndexPath *index = notification.userInfo[@"index"];
        NSLog(@"这个页面的indexpath 有问题 == %@", index);
        if (!self.dataSource.count) {   //数组删除数目时出现错误,为空了,取不到对应的下标
            [self requsetData];
        }else{
            if (self.dataSource.count <= index.row ) {
                [self.tableView reloadData];
                return;
            }
            ...
            [WPHttpTool postWithURL:url params:params success:^(id json) {
                ...
                if (model.list.count) {
                    NSLog(@"一开始当前数组的元素个数为 ==%lu", (unsigned long)self.dataSource.count);
                    [self.dataSource removeObjectAtIndex:index.row];
                      NSLog(@"进行中当前数组的元素个数为 ==%lu", (unsigned long)self.dataSource.count);
                    [self.dataSource insertObject:model.list[0] atIndex:0];
                    NSLog(@"结束后当前数组的元素个数为 ==%lu", (unsigned long)self.dataSource.count);
                }else{
                     [MBProgressHUD createHUD:@"操作过于频繁!" View:self.view];
                }
                [self.tableView reloadData];
