...
-(void)setContent:(NSString *)name AvatarImage:(NSString *)urlString
{
    ...
    if ([urlString isEqualToString:@"tt_group_manager_add_user.jpg"]) {
        self.tag=10000;
        self.personIcon.image = [UIImage imageNamed:@"tt_group_manager_add_user"];
    }else if([urlString isEqualToString:@"tt_group_manager_delete_user.jpg"])
    {
        self.tag=100001;
        self.personIcon.image = [UIImage imageNamed:@"tt_group_manager_delete_user"];
    }else
    {
        [self.personIcon sd_setImageWithURL:[NSURL URLWithString:[IPADDRESS stringByAppendingString:urlString]] placeholderImage:[UIImage imageNamed:@"user_placeholder"]];
    }
