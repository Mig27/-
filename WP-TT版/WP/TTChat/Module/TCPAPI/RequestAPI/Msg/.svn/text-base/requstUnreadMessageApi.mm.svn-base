//
//  requstUnreadMessageApi.m
//  WP
//
//  Created by CC on 16/10/21.
...
@implementation requstUnreadMessageApi
/**
 *  请求超时时间
 *
 *  @return 超时时间
...
//            NSString *latest_msg_content = unreadInfo.latestMsgData;
            MTTSessionEntity *session = [[MTTSessionEntity alloc] initWithSessionID:userID type:(sessionType==1)?SessionTypeSessionTypeSingle:SessionTypeSessionTypeGroup];
            session.unReadMsgCount=unread_cnt;
//            session.lastMsg=latest_msg_content;
            session.lastMsgID=latest_msg_id;
            session.lastMsgFromUserId = unreadInfo.latestMsgFromUserId;
            NSString * string = [[NSString alloc]initWithData:unreadInfo.latestMsgData encoding:NSUTF8StringEncoding];
            
            if (string.length) {
                if (unreadInfo.latestMsgType == MsgTypeMsgTypeSingleAudio || unreadInfo.latestMsgType == MsgTypeMsgTypeGroupAudio) {
