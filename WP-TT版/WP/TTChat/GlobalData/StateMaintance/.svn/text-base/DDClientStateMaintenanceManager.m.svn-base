...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:DD_NETWORK_STATE_KEYPATH])
    {
        ...
        {
            //发出通知上传未完成的说说
            [[NSNotificationCenter defaultCenter] postNotificationName:@"UPLOADSHUOSHUOAGAIN" object:nil];
            NSString * oldStr = [NSString stringWithFormat:@"%@",change[@"old"]];
            if ([oldStr isEqualToString:@"3"]) {
              [[NSNotificationCenter defaultCenter] postNotificationName:@"refreshShuoShuo" object:nil];
            }
            if ([DDClientState shareInstance].userState != DDUserOnline)
            {
//                //从新登陆时发送通知
                [[NSNotificationCenter defaultCenter] postNotificationName:@"Relogining" object:nil];
            }
        ...
        else
        {
            ...
            [RecentUsersViewController shareInstance].title=@"消息";//连接失败
            [[NSNotificationCenter defaultCenter] postNotificationName:@"OFFLINEOFNETWORK" object:nil];
        }
    }
    //用户状态变化
    else if ([keyPath isEqualToString:DD_USER_STATE_KEYPATH])
    {
        switch ([DDClientState shareInstance].userState)
        {
            case DDUserKickout:
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:@"OFFLINEOFNETWORK" object:nil];//掉线通知
                break;
            case DDUserOffLine:
                [RecentUsersViewController shareInstance].title=@"消息";
                [self p_stopCheckServerHeartBeat];
                [self p_stopHeartBeat];
                [self p_startRelogin];
                [[NSNotificationCenter defaultCenter] postNotificationName:@"OFFLINEOFNETWORK" object:nil];
            ...
            case DDUserOffLineInitiative:
              ...
              [[NSNotificationCenter defaultCenter] postNotificationName:@"OFFLINEOFNETWORK" object:nil];
                break;
            case DDUserOnline:
                [RecentUsersViewController shareInstance].title=@"消息";//APP_NAME
                ...
                break;
            case DDUserLogining:
                [RecentUsersViewController shareInstance].title=@"消息";//收取中
//                [[NSNotificationCenter defaultCenter] postNotificationName:@"Relogining" object:nil];
                break;
        }
    }
