...
+(MTTMessageEntity*)makeMessage:(NSString*)content session:(NSString*)sessionId MsgType:(DDMessageContentType)type
{
    ...
    MsgType msgType;
    msgType =MsgTypeMsgTypeSingleText;
    MTTMessageEntity* message = [[MTTMessageEntity alloc] initWithMsgID:[DDMessageModule getMessageID] msgType:msgType msgTime:msgTime sessionID:sessionId senderID:senderID msgContent:content toUserID:sessionId];
    message.state = DDMessageSending;
    message.msgContentType=type;
    return message;
}
-(NSString*)getStrFrom:(NSString*)string
{
...
-(BOOL)isPersonCard
{
    if (self.msgContentType == DDMEssagePersonalaCard) {
        return YES;
    }
    return NO;
}
