...
//开始录制声音
- (void)startRecording {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"disEnableInterActivity" object:nil];
...
- (void)timeoutCheck:(id)sender {
    ...
    //超时是自动发送，停止记录
    if (self.delegate) {
        dateStopRecording = [NSDate date];
        [self.delegate recordingFinishedWithFileName:filename time:60];
    }
