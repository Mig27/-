	...
	for (int i=0; i<DD_EMPTIONS_ROWS; i++) {
		...
		for (int y=0; y<DD_EMOTIONS_PERROW; y++) {
            ...
            if ((i * DD_EMOTIONS_PERROW + y + page * DD_MAC_EMOTIONS_COUNT_PERPAGE) > [emotions count]) {
                ...
                {
                    [button setImage:[UIImage imageNamed:@"chat_biaoqing_shanchu"] forState:UIControlStateNormal];//dd_emoji_delete
                    button.tag=10000;
                    [button setImage:[UIImage imageNamed:@"chat_biaoqing_shanchu_pre"] forState:UIControlStateHighlighted];

                    [button setImageEdgeInsets:UIEdgeInsetsMake(button.width/6, button.width/6, button.width/6, button.width/6)];
