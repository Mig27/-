...
{
    _LewVideoController_Net *videoController = [[_LewVideoController_Net alloc]initWithcontentFile:filePath];
//    [videoController _addProgressObserver];
//    [videoController _addPlayerStausObserver];
//    [videoController _addLoadedProcessObserver];
    return videoController;
...
- (void)cancel{
    [_player pause];
    if (!self.isCreat||(self.isNetOrNot && self.isCreat)) {
        [_player.currentItem removeObserver:self forKeyPath:@"status"];
        [_player.currentItem removeObserver:self forKeyPath:@"loadedTimeRanges"];
    }
//    if (self.isNetOrNot && self.isNetOrNot) {
//        [_player.currentItem removeObserver:self forKeyPath:@"status"];
//        [_player.currentItem removeObserver:self forKeyPath:@"loadedTimeRanges"];
//    }
...
-(instancetype)initWithcontentFile:(NSString*)filePath
{
    if (self = [super init]) {
        ...
        [self.player play];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(runLoopVideo:) name:AVPlayerItemDidPlayToEndTimeNotification object:nil];
    }
    return self;
    
}
-(void)runLoopVideo:(NSNotification*)notification
{
